...
void CRadiator::GenerateCellArray_RoundH(float _phic) {
	int i, j, k;
	float tempR;
	FILE* Checkmodel;
	//Position
	...
	for (k = 0; k < Nz_model; k++) {
		for (j = 0; j < Ny_model; j++) {
			for (i = 0; i < Nx_model; i++) {
				if (RPECStructure(px[i], py[j], pz[k], _phic)) {
					EpsMap[k][j][i] = 0;
					Esig[k][j][i] = Sig_PECf;
				}
				else {
	...
	Checkmodel = fopen("./modelmark.dat", "wb");
	//这里输出三维模型在三个维度的数目
	fwrite(&Nx_model, sizeof(int), 1, Checkmodel);
	fwrite(&Ny_model, sizeof(int), 1, Checkmodel);
	fwrite(&Nz_model, sizeof(int), 1, Checkmodel);
	//这里输出激励场在三维模型中的相对位置
	fwrite(&Px_model, sizeof(int), 1, Checkmodel);
	fwrite(&Py_model, sizeof(int), 1, Checkmodel);
	//discrete intervel
	fwrite(&dx, sizeof(float), 1, Checkmodel);
	fwrite(&dy, sizeof(float), 1, Checkmodel);
	fwrite(&dz, sizeof(float), 1, Checkmodel);
	//centerPosition
	pos = 0.5*(px[0] + px[Nx_model - 1]);//xCenter
	fwrite(&pos, sizeof(float), 1, Checkmodel);

	pos = 0.5*(py[0] + py[Ny_model - 1]);//yCenter
	fwrite(&pos, sizeof(float), 1, Checkmodel);

	pos = 0.5*(pz[0] + pz[Nz_model - 1]);//zCenter
	fwrite(&pos, sizeof(float), 1, Checkmodel);
	...
	for (k = 0; k < Nz_model; k++) {
		for (j = 0; j < Ny_model; j++) {
			for (i = 0; i < Nx_model; i++) {
				tempf = (1 - EpsMap[k][j][i])*1.0;
				fwrite(&tempf, sizeof(float), 1, Checkmodel);
			}
		}
	}
	fclose(Checkmodel);
...
float CRadiator::RFxy(float x, float y,float _phic) {
	float z;
	float pi2 = PIf * 2;
	float phi = atan2(y, x);
	...
	if (Rotation == 1) phi = pi2 - phi;
	else if (Rotation == 2) phi = phi;
	if (phi > pi2) phi = phi - pi2;
	else if (phi < 0) phi = phi + pi2;
	z = Lc * phi / (pi2);

	return z;
}

bool CRadiator::RPECStructure(float x, float y, float z, float _phic) {
	bool PEC = false;
	float rr;
	float thick = 5*dx;
	float top;
	rr = sqrt(x*x + y*y);
	top = RFxy(x, y, _phic) + Lp;
